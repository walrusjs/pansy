...
async function printAssets(assets: Assets, title: string) {
  ...
  const table = await Promise.all(
    [...assets.keys()].map(async (relative) => {
      ...
      return [chalk.green(relative), prettyBytes(size), prettyBytes(await gzipSize(asset.source))];
    })
  );
...
export default async function createRollupConfig (
  ...
  const pluginsOptions: { [key: string]: any } = {
    ...
    hashbang: config.plugins.hashbang !== false && lodash.merge({}, config.plugins.hashbang),
  ...
  plugins.push({
    ...
    async writeBundle() {
      await printAssets(
        assets,
        `${title.replace('Bundle', 'Bundled')} ${chalk.dim(`(${formatTime(endTime - startTime)})`)}`
      );
    }
  });
  ...
  return {
    inputConfig: {
      ...
      onwarn(warning) {
        ...
        logger.warn(`${chalk.yellow(`${code}`)}${chalk.dim(':')} ${message}`);
      }
    },
    outputConfig: {
